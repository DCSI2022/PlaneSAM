...
class EfficientSam(nn.Module):
    ...
    def __init__(
        ...
        pixel_mean: List[float] = [0.485, 0.456, 0.406, 0.473],
        pixel_std: List[float] = [0.229, 0.224, 0.225, 0.189],
    ) -> None:
        """
        ...
        self.register_buffer(
            "pixel_mean", torch.Tensor(pixel_mean).view(1, 4, 1, 1), False
        )
        self.register_buffer(
            "pixel_std", torch.Tensor(pixel_std).view(1, 4, 1, 1), False
        )
    ...
    def predict_masks(
        ...
        if num_pts > self.decoder_max_num_input_points:
            ...
            # pad里每两个元素是一组，表示在第1,2,3...个维度的前后填充的个数
    ...
    @torch.jit.export
    def get_image_embeddings(self, batched_images, batched_depths) -> torch.Tensor:
        """
        ...
        Arguments:
          ...
          batched_images: A tensor of shape [B, 1, H, W]
        Returns:
          List of image embeddings each of of shape [B, C(i), H(i), W(i)].
          The last embedding corresponds to the final layer.
        """
        batched_rgbd = self.preprocess(torch.cat((batched_images, batched_depths), dim=1))
        batched_images = batched_rgbd[:, :3, ...]
        batched_depths = batched_rgbd[:, 3:, ...]
        return self.image_encoder(batched_images, batched_depths)

    def forward(
        self,
        batched_images: torch.Tensor,
        batched_depths: torch.Tensor,
        batched_points: torch.Tensor,
        batched_point_labels: torch.Tensor,
        ...
        Arguments:
          batched_images: A tensor of shape [B, 3, H, W]
          batched_depths: A tensor of shape [B, 1, H, W]
          batched_points: A tensor of shape [B, num_queries, max_num_pts, 2],每一个提示可以对应多个点
        ...
        # [1, 256, 64, 64]
        image_embeddings = self.get_image_embeddings(batched_images, batched_depths)
...
def build_efficient_sam(encoder_patch_embed_dim, encoder_num_heads, checkpoint=None):
    ...
    sam = EfficientSam(
        ...
        pixel_mean=[0.485, 0.456, 0.406, 0.473],
        pixel_std=[0.229, 0.224, 0.225, 0.189],
    )
